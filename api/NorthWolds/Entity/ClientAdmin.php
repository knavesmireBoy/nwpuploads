        ...
        return function (?int $cid, array $postdata, int $insertID = 0) use ($key) {
            ...
            if (isset($dbrecord['email'])) { //existing
                ...
                if ($cid && $cid != $dbrecord['client_id']) {
                    ...
                    $getDoms = composer(curry2('in_array')($dom), curry2('array_filter')($f), partial('array_values'), partial('array_map', curry2('getter')(0)), partial('array_map', 'parseEmail'), partial('array_column', $clients));
                    $domz = $getDoms('domain');
